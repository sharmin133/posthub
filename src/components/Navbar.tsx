"use client";

import { useState } from "react";
import Link from "next/link";
import { useSession, signIn, signOut } from "next-auth/react";
import { HiMenu, HiX } from "react-icons/hi"; // Hamburger & Close icons

export default function Navbar() {
  const { data: session } = useSession();
  const [isOpen, setIsOpen] = useState(false);

  return (
    <nav className="bg-blue-600 text-white md:p-6 py-7 fixed w-full top-0 z-50 shadow">
      <div className="flex justify-between items-center">
        {/* Left: Logo */}
        <div className="text-xl font-bold">
          <Link href="/">PostHub</Link>
        </div>

        {/* Desktop Menu */}
        <div className="hidden md:flex items-center gap-4">
          <Link href="/" className="hover:text-gray-200">
            Home
          </Link>
          <Link href="/dashboard" className="hover:text-gray-200">
            Dashboard
          </Link>
          {session && (
            <Link href="/profile" className="hover:text-gray-200">
              Profile
            </Link>
          )}
          {!session ? (
            <button
              onClick={() => signIn("google")}
              className="bg-white text-blue-600 px-4 py-1 rounded"
            >
              Sign In
            </button>
          ) : (
            <button
              onClick={() => signOut()}
              className="bg-white text-blue-600 px-4 py-1 rounded"
            >
              Logout
            </button>
          )}
        </div>

        {/* Mobile Menu Button */}
        <button
          className="md:hidden text-2xl focus:outline-none"
          onClick={() => setIsOpen(!isOpen)}
        >
          {isOpen ? <HiX /> : <HiMenu />}
        </button>
      </div>

      {/* Mobile Dropdown */}
      {isOpen && (
        <div className="md:hidden mt-2 flex flex-col gap-2 bg-blue-700 p-3 rounded-lg">
          <Link
            href="/"
            className="hover:text-gray-200"
            onClick={() => setIsOpen(false)}
          >
            Home
          </Link>
          <Link
            href="/dashboard"
            className="hover:text-gray-200"
            onClick={() => setIsOpen(false)}
          >
            Dashboard
          </Link>
          {session && (
            <Link
              href="/profile"
              className="hover:text-gray-200"
              onClick={() => setIsOpen(false)}
            >
              Profile
            </Link>
          )}
          {!session ? (
            <button
              onClick={() => {
                signIn("google");
                setIsOpen(false);
              }}
              className="bg-white text-blue-600 px-4 py-1 rounded"
            >
              Sign In
            </button>
          ) : (
            <button
              onClick={() => {
                signOut();
                setIsOpen(false);
              }}
              className="bg-white text-blue-600 px-4 py-1 rounded"
            >
              Logout
            </button>
          )}
        </div>
      )}
    </nav>
  );
}
